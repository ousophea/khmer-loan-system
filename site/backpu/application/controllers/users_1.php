...
class users extends CI_Controller {
    ...
    function __construct() {
        parent::__construct();
        $this->load->model(array('s_users', 'd_users', 'd_roles', 's_roles', 'm_cofficer', 'm_global'));
...
//                    $this->session->set_userdata('bra_id', $roles->bra_id );
//                    echo  $this->session->userdata('use_bra_id');exit();
...
//                    $this->input->set_cookie($cookie);
    ...
    function register() {
        try {
            allows(array(Setting::$role0, Setting::$role4));
            ...
            if ($this->form_validation->run() == FALSE) {
                ...
                $this->data['brands'] = $this->get_dropdown($brands, 'bra_id', 'bra_name');
                ...
                if ($this->d_users->getRegister($s_user_obj)) {
                    //todo add branch
                    $uob_use_id = $this->d_users->insert_id();
                    $branches = $this->input->post('branches');

                    if (count($branches) > 0) {
                        $data = array();
                        foreach ($branches as $branch) {
                            $data[] = array($uob_use_id, $branch);
                        }
                        $this->m_global->insert_multi('user_of_branch', array('uob_use_id', 'uob_bra_id'), $data);
                    }
    ...
    function get_dropdown($data = array(), $field_key, $field_value) {
        $res = array();
        foreach ($data as $row) {
    ...
    function edit() {
        ...
        try {
            ...
            if ($this->form_validation->run() == FALSE) {
                ...
                $this->data['roles'] = $this->d_roles->getAllRoles($obj);
                $this->data['data'] = array('user' => $this->d_users->getUserById($co_id));

                $brands = $this->m_global->select_data_join('branch', 'bra_id,bra_name', array(), array('bra_status' => 1));
                $this->data['arr_branch'] = $this->get_dropdown($brands, 'bra_id', 'bra_name');

                $this->data['select_branch'] = $this->m_global->select_data_join(
                        'user_of_branch', '*', array('branch' => array('uob_bra_id' => 'bra_id')), array('uob_use_id' => $co_id, 'uob_status' => 1, 'bra_status' => 1));
                ...
                if ($this->d_users->editUserById()) {
                     $co_id = $this->uri->segment(3);
                    if ($this->checkUpdateBranch($co_id)) {
    ...
    function checkUpdateBranch($use_id) {
        $branches = $this->input->post('branches');
        $result = false; // to check if can update the branch of user or not
        if (count($branches) > 0) {
            $tmp = $this->m_global->select_data_join('user_of_branch', '*', array(), array('uob_use_id' => $use_id, 'uob_status' => 1));
            //todo update what we don't have
            foreach ($tmp as $item) {
                $_uob_use_id = $item['uob_use_id'];
                $_uob_bra_id = $item['uob_bra_id'];
                if (!in_array($_uob_bra_id, $branches)) {
                    $this->m_global->update('user_of_branch', array('uob_status' => 0), array('uob_use_id' => $_uob_use_id, 'uob_bra_id' => $_uob_bra_id));
                }
            }
            $data = array();
            foreach ($branches as $branch) {
                if (!$this->checkInBranch($branch, $tmp)) {
                    $data[] = array($_uob_use_id, $branch);
                }
            }
            if ($data) {
                $result = $this->m_global->insert_multi('user_of_branch', array('uob_use_id', 'uob_bra_id'), $data);
            }
            return $result;
        }
    }
    ...
    function manage() {
        allows(array(Setting::$role0, Setting::$role1, Setting::$role4));
